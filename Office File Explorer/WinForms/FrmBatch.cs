  ...
  public partial class FrmBatch : Form
  {
    ...
    public void PopulateAndDisplayFiles()
    {
      try
      {
        ...
        foreach ( FileInfo f in dir.GetFiles( "*.*", ckbSubfolders.Checked ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly ) )
        {
          ...
          {
            bool isOldDOC = cbIncludeOldDOC.Checked && f.Extension.ToLowerInvariant() == ".doc";
            if ( GetFileExtension() == "*.docx" || isOldDOC )
            {
              if ( isOldDOC || f.Name.EndsWith( Strings.docxFileExt ) || f.Name.EndsWith( Strings.docmFileExt ) || f.Name.EndsWith( Strings.dotxFileExt ) || f.Name.EndsWith( Strings.dotmFileExt ) )
              {
    ...
    private void cbIncludeOldDOC_CheckedChanged( object sender, EventArgs e )
    {
      if( lstOutput.Items.Count>0 )
      try
      {
